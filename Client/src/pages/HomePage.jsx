...
// Component: IconSet
// A collection of SVG icon components for easy reuse throughout the page.
const IconSet = {
...
// A reusable wrapper component that animates its children (fades and slides in)
// when they scroll into the browser's viewport.
const AnimatedSection = ({ children, direction = 'up' }) => {
  // State to track if the component is visible on screen
  const [isVisible, setIsVisible] = useState(false);
  // Ref to attach to the DOM element for the observer to watch
  const ref = useRef(null);

  useEffect(() => {
    // Create an IntersectionObserver to watch for when the element enters the viewport
    const observer = new IntersectionObserver(
      ([entry]) => {
        // Update state based on whether the element is intersecting (visible) or not
  ...
  // Function to determine the correct Tailwind classes based on visibility and direction prop
...
const HomePage = () => {
  ...
  const [wordIndex, setWordIndex] = useState(0); // Index of the current word from the array
  const [text, setText] = useState(''); // The currently displayed text
  const [isDeleting, setIsDeleting] = useState(false); // Tracks if the animation is deleting
  const wordsToType = ["Innovation", "Collaboration", "Creation"]; // Array of words to cycle through

  useEffect(() => {
    // This function handles the logic for typing and deleting characters
    const handleTyping = () => {
      const currentWord = wordsToType[wordIndex];
      const updatedText = isDeleting
        ? currentWord.substring(0, text.length - 1) // Delete character
        : currentWord.substring(0, text.length + 1); // Add character

      setText(updatedText);

      // Logic to switch between typing, pausing, and deleting
      if (!isDeleting && updatedText === currentWord) {
        setTimeout(() => setIsDeleting(true), 1500); // Pause after word is typed
      } else if (isDeleting && updatedText === '') {
        setIsDeleting(false);
        setWordIndex((prevIndex) => (prevIndex + 1) % wordsToType.length); // Move to the next word
      }
    };
  ...
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 }); // State to store cursor coordinates
  ...
  // Calculate normalized offsets for parallax effect (-1 to 1 range)
      ...
      <main className="pt-24 md:pt-32">
        ...
        <section className="container mx-auto px-6 py-16">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            {/* --- Hero Section: Left Side (Text Content) --- */}
            ...
            {/* --- Hero Section: Right Side (Cursor-Reactive Visual Element) --- */}
            <div className="relative h-64 md:h-96">
              <div
                className="absolute w-full h-full grid grid-cols-3 grid-rows-3 gap-4 transition-transform duration-300 ease-out"
                style={{ transform: `translateX(${xOffset * -10}px) translateY(${yOffset * -10}px) rotate(-6deg)` }}
              >
        ...
        {/* Section: Trusted By (with scroll animation) */}
        ...
        {/* Section: Features (with scroll animation) */}
        ...
        {/* Section: How it Works (with alternating scroll animations) */}
        ...
        {/* Section: Final Call to Action (with scroll animation) */}
